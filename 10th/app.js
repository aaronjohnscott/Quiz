/* ============================================================
   Cosmic Compass! - quiz engine (no backend, all local)
   ============================================================ */

(function () {
  'use strict';

  const STORAGE_KEY = 'cosmic-compass-quiz-v1';
  ...
  /* ---------- answer shuffling ----------
     Answers are authored by archetype but shown in a scrambled order, so slot 1
     isn't always the same archetype. The shuffle for each question is
     ...
     what the scoring reads â€” the display order never touches the points. */
  ...
  function save() {
    try {
      ...
      /* private browsing / storage full â€” the quiz still works, just no resume */
    }
  }
  ...
  /* ---------- motion switch ----------
     Some students find the drifting leaves and ripples distracting, so the
     whole ambient layer can be shut off from the bar at the bottom. The
     choice is remembered for next time. */

  const MOTION_KEY = 'cosmic-compass-motion';
  let motionOn = true;
  ...
  function applyMotion(on) {
    ...
    el.motionToggle.title = on
        ? 'Turn off the cosmic glow'
        : 'Turn the cosmic glow back on';
  }
  ...
  function renderQuestion() {
    ...
    if (q.section) {
      ...
      el.qSection.hidden = false;
    } else {
      el.qSection.hidden = true;
    }
    ...
    el.nextBtn.textContent =
      state.index === QUESTIONS.length - 1 ? 'See my result âœ¦' : 'Next â†’';
  ...
  function renderWritten(q, stored) {
    ...
    el.textarea.placeholder = q.placeholder || 'Type your answer hereâ€¦';
  ...
  // Written answers are never gated â€” the whole point is to see what a
  ...
  // 1â€“4 picks an option, Enter advances â€” but never while typing an essay.
  ...
  function scoreQuiz() {
    const scores = { stargazer: 0, missioncontrol: 0, orbitmate: 0, trailblazer: 0 };
  ...
  /* Builds the printout. Students never see this on screen â€” it exists so the
  ...
  // cosmic_compass_quiz_2026-09-08_student.pdf - sorts by date and says whose
  ...
  function quizFileName(extension) {
    ...
    return 'cosmic_compass_quiz_' + stamp + '_' + who + '.' + extension;
  }

  /* ============================================================
     PNG FALLBACK

     This is drawn from the stored answers onto a canvas â€” it is not a
     screenshot of the results page. That's deliberate: a picture of
     "You are a Stargazer!" proves nothing, so the image is a full answer sheet
  ...
  function paintSheet(ctx, result, draw) {
    ...
    line('Cosmic Compass!', 'bold 30px Georgia, serif', SHEET.accent, 34);
    y += 8;
    ...
    line(
      'Points: ' + SCORE_ORDER.map((k) => TYPES[k].name + ' ' + result.scores[k]).join('  Â·  '),
  ...
  function savePng() {
    ...
    // measure first â€” the sheet is as tall as the writing demands
    ...
    function deliver(url, revoke) {
      ...
      flash(el.pngBtn, 'Image saved âœ“');
    }
  ...
  /* Builds the PDF in the page and hands it to the browser as a download, so
     there's no print dialog and nothing to name â€” it lands in Downloads as
     cosmic_compass_quiz_2026-09-08_student.pdf and is ready to upload. */
  function savePdf() {
    // pdf.js didn't load â€” nearly always a stale cached copy of index.html
    if (typeof LakePdf === 'undefined') {
      el.helpnote.querySelector('.helpnote__title').textContent =
        'âš ï¸ The page didnâ€™t finish loading';
    ...
    try {
      ...
      const bytes = LakePdf.build({
        title: 'Cosmic Compass! - Find Your Cosmic Archetype',
        meta: [
          'Name: ' + state.name,
      ...
      flash(el.pdfBtn, 'âœ“ PDF saved to your Downloads');
    } catch (err) {
      reportTrouble(String(err));
    }
  }
  ...
  function openFallback() {
    ...
    el.troubleBtn.textContent = 'Backup options are open below â†“';
  ...
  /* ---------- clipboard lock ----------
     ...
     every route into the box â€” Ctrl+V, the right-click menu, and middle-click
     â€” while `dragstart`/`drop` stop text being dragged in or out. Selection
  ...
  // The warning stays out of sight until someone actually tries to paste â€”
