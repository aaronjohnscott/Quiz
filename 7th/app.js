/* ============================================================
   Beach Bound! — quiz engine (no backend, all local)
   ============================================================ */

(function () {
  'use strict';

  const STORAGE_KEY = 'beach-bound-quiz-v1';
  ...
  /* ---------- answer shuffling ----------
     Answers are authored shell/starfish/dolphin/wave but shown in a scrambled
     order, so slot 1 isn't always Shell. The shuffle for each question is
  ...
  /* ---------- motion switch ----------
     Some students find the drifting shells and bubbles distracting, so the
     whole ambient layer can be shut off from the bar at the bottom. The
     choice is remembered for next time. */

  const MOTION_KEY = 'beach-bound-motion';
  let motionOn = true;
  ...
  function applyMotion(on) {
    ...
    el.motionToggle.title = on
      ? 'Turn off the drifting shells and bubbles'
      : 'Turn the drifting shells and bubbles back on';
  }
  ...
  function scoreQuiz() {
    const scores = { shell: 0, starfish: 0, dolphin: 0, wave: 0 };
  ...
  // beach_quiz_2026-08-05_avery-chen.pdf — sorts by date and says whose
  ...
  function quizFileName(extension) {
    ...
    return 'beach_quiz_' + stamp + '_' + who + '.' + extension;
  }
  ...
  /* ============================================================
     ...
     "You are a Wave!" proves nothing, so the image is a full answer sheet
  ...
  function paintSheet(ctx, result, draw) {
    ...
    line('Beach Bound!', 'bold 30px Georgia, serif', SHEET.accent, 34);
    y += 8;
  ...
  /* Builds the PDF in the page and hands it to the browser as a download, so
     ...
     beach_quiz_2026-08-05_avery-chen.pdf and is ready to upload. */
  function savePdf() {
    // pdf.js didn't load — nearly always a stale cached copy of index.html
    if (typeof BeachPdf === 'undefined') {
    ...
    try {
      ...
      const bytes = BeachPdf.build({
        title: 'Beach Bound! - Discover Your Island Personality',
        meta: [
          'Name: ' + state.name,
