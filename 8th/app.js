/* ============================================================
   Lake Life! — quiz engine (no backend, all local)
   ============================================================ */

(function () {
  'use strict';

  const STORAGE_KEY = 'lake-life-quiz-v1';
  ...
  const MOTION_KEY = 'lake-life-motion';
  let motionOn = true;
  ...
  function applyMotion(on) {
    ...
    el.motionToggle.title = on
      ? 'Turn off the drifting leaves and ripples'
      : 'Turn the drifting leaves and ripples back on';
  }
  ...
  function scoreQuiz() {
    const scores = { driftwood: 0, lilypad: 0, duck: 0, canoe: 0 };
  ...
  // lake_quiz_2026-08-05_avery-chen.pdf — sorts by date and says whose
  ...
  function quizFileName(extension) {
    ...
    return 'lake_quiz_' + stamp + '_' + who + '.' + extension;
  }
  ...
  function paintSheet(ctx, result, draw) {
    ...
    line('Lake Life!', 'bold 30px Georgia, serif', SHEET.accent, 34);
    y += 8;
  ...
  /* Builds the PDF in the page and hands it to the browser as a download, so
     ...
     lake_quiz_2026-08-05_avery-chen.pdf and is ready to upload. */
  function savePdf() {
    ...
    try {
      ...
      const bytes = LakePdf.build({
        title: 'Lake Life! - Discover Your Lakeside Personality',
        meta: [
          'Name: ' + state.name,
