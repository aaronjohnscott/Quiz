/* ============================================================
   Lake Life! â€” quiz engine (no backend, all local)
   ============================================================ */

(function () {
  'use strict';

  const STORAGE_KEY = 'urban-pulse-quiz-v1';
  ...
  /* ---------- answer shuffling ----------
     Answers are authored driftwood/lilypad/duck/canoe but shown in a scrambled
     order, so slot 1 isn't always Driftwood. The shuffle for each question is
  ...
  const MOTION_KEY = 'urban-pulse-motion';
  let motionOn = true;
  ...
  function applyMotion(on) {
    ...
    el.motionToggle.title = on
        ? 'Turn off ambient city motion'
        : 'Turn ambient city motion back on';
  }
  ...
  function scoreQuiz() {
    const scores = { navigator: 0, planner: 0, connector: 0, builder: 0 };
  ...
  // city_pulse_quiz_2026-09-08_student.pdf â€” sorts by date and says whose
  ...
  function quizFileName(extension) {
    ...
    return 'city_pulse_quiz_' + stamp + '_' + who + '.' + extension;
  }
  ...
  /* ============================================================
     ...
     "You are a Canoe!" proves nothing, so the image is a full answer sheet
  ...
  function paintSheet(ctx, result, draw) {
    ...
    line('Urban Pulse!', 'bold 30px Georgia, serif', SHEET.accent, 34);
    y += 8;
  ...
  /* Builds the PDF in the page and hands it to the browser as a download, so
     ...
     city_pulse_quiz_2026-09-08_student.pdf and is ready to upload. */
  function savePdf() {
    ...
    try {
      ...
      const bytes = LakePdf.build({
        title: 'Urban Pulse! - Find Your City Archetype',
        meta: [
          'Name: ' + state.name,
